...
Future<int?> getGameRecord(String userId, String gameId) async {
  try {
    ...
    if (snapshot.exists) {
      ...
      // Verifica si 'records' es un mapa y contiene el récord del juego específico
      if (data.containsKey('records') && data['records'] is Map<String, dynamic>) {
        ...
        if (records.containsKey(gameId)) {
          int record = records[gameId]; // Accede al campo correspondiente al gameId
          print('Récord recuperado desde Firestore para $gameId: $record');
          return record;
        } else {
          print('No se encontró un récord para $gameId, devolviendo 0');
          return 0; // Si no se encuentra el campo para el juego, devuelve 0
        }
      } else {
